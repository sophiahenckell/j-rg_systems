...
class StructureFactor(object):
    ...
    def __init__(self,box_size,n_atoms,big_array_r,big_array_q,q_vector):
        self.L = box_size
        self.qindex = round(self.L*q_vector/2./np.pi)
        self.natoms = n_atoms
    ...
    def calculate_incoherent_sk(self):
        ...
        for timeframe in displacement:
            ...
            for particledelta in timeframe:
                ...
                inc_part = (np.sum([ exp(-1j * round(np.dot(q,particledelta) * fac,6)) for q in self.maxq ])) # np.sum(np.dot(q,particledelta))
                
                incoherent_part_t += np.real(inc_part)/self.natoms # normalize with number of atoms
            # get the average over all atoms
            incoherent_part.append(incoherent_part_t/number_of_q_vectors)
            print(incoherent_part_t,inc_part,inc_part*np.conj(inc_part))
...
#                 print('----',q,rho)
        ...
        qind = np.round(self.qindex) #13i,11
        print('quind', qind)
        qind = int(qind)
        print('qindex', qind)
        print("index",qind,q_find_max[qind],max(sk_find_max))
        print("range",q_find_max[qind-1],q_find_max[qind],q_find_max[qind+1])
        self.max_sq_static.append(q_find_max[qind-4])
        self.max_sq_static.append(q_find_max[qind-3])
        ...
        for q in q_data:
            ...
            if (s >= q_binreal[qind-1]) and (s <= q_binreal[qind+1]): # get two-four bins before and after to set the range
                self.maxq.append(q)
...
def calc_sk(path, brownian_timestep, saving_timestep, system_lifetime, packing_fraction, particle_number, aspect_ratio, simulation_id, q_vector, verbose):
    """
    ...
    # calculate the ff:
    calculate_sk = StructureFactor(L,particle_number,rbig,qbig,q_vector)
    ...
    sk_static_dat = open(filename+'-static-sk_new.dat','w')
    print(sk_static_dat)
    sk_incoherent_part_dat = open(filename+'-incoherent-sk_new.dat','w')
    print(sk_incoherent_part_dat)
...
#          
#    calc_sk(args.path, args.brownian_timestep, args.saving_timestep, args.system_lifetime, args.packing_fraction, args.particle_number, args.aspect_ratio, args.simulation_id, args.q_vector, args.verbose)
    

    parser = argparse.ArgumentParser(description='Perform a Brownian simulation of the Woetzel benchmark ellipse system.')
    simgroup = parser.add_argument_group('Simulation settings')
    simgroup.add_argument('--id', metavar='id', help='Path to save simulation output to.', required=True)
    args = parser.parse_args()
    path = '/data/scc/sophia/Figure4_2_StatStrucFact_MSDCalculations1-n1372-t2000-phi0.494-k0-tb0.01-ts5'
    calc_sk(path, 0.01, 5, 2000, 0.494, 1372, 0.0, args.id, 7, True )    
#    calc_sk(path,0.01, 5, 2000, 0.494, 1372, 0.0, 1, 7, True )
