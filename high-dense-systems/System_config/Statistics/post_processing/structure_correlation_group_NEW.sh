...
cat > $TMPFILE <<- EOF
	...
	args="--path $DATA --brownian-timestep ${BROWNIAN_TIMESTEP} --saving-timestep ${SAVING_TIMESTEP} --system-lifetime ${SIMULATION_TIME} --packing-fraction ${PACKING_FRACTION} --particle-number ${PARTICLE_NUMBER} --aspect-ratio ${ASPECT_RATIO} --simulation-id \$SGE_TASK_ID --q-vector ${Q_VECTOR} --verbose"
	echo \${cmd} \${args}
	exec \${cmd} \${args}
EOF
