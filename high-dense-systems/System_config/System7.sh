...
PACKING_FRACTION=0.44
PARTICLE_NUMBER=1372
ASPECT_RATIO=0
RADIUS=1
 

NAME="SYSTEM7${RUN_ID}T${SIMULATION_TIME}N${PARTICLE_NUMBER}k${ASPECT_RATIO//.}Phi${PACKING_FRACTION//.}"
DATA="/data/scc/sophia/PaperSystems/SYSTEM7/${RUN_ID}-n${PARTICLE_NUMBER}-t${SIMULATION_TIME}-phi${PACKING_FRACTION}-k${ASPECT_RATIO}-tb${BROWNIAN_TIMESTEP}-ts${SAVING_TIMESTEP}-r${RADIUS}"
mkdir -p $DATA/logs
