...
PARTICLE_NUMBER=1600
SIMULATION_TIME=100
ASPECT_RATIO=1.2
BROWNIAN_TIMESTEP=0.1
SAVING_TIMESTEP=1
SWELLING_RATE=10
NAME="swell${RUN_ID}T${SIMULATION_TIME}N${PARTICLE_NUMBER}k${ASPECT_RATIO//.}Phi${PACKING_FRACTION//.}"
DATA="/data/scc/sophia/mw-benchmarks_spheres/mw-run${RUN_ID}-n${PARTICLE_NUMBER}-t${SIMULATION_TIME}-phi${PACKING_FRACTION}-k${ASPECT_RATIO}-tb${BROWNIAN_TIMESTEP}-ts${SAVING_TIMESTEP}"
mkdir -p $DATA/logs
