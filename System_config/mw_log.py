...
def start(path, brownian_timestep, saving_timestep, swelling_rate, system_lifetime, packing_fraction, particle_number, aspect_ratio, radius, simulation_id,verbose):
    ...
    sim = Simulation(config_filename=config_filename, output_filename=config_filename, system_lifetime=100, brownian_timestep=None,  saving_timestep=saving_timestep, swelling_rate = None, packing_fraction = packing_fraction, aspect_ratio = aspect_ratio, verbose=verbose)
    sim.start()
    sys.exit()
...
if __name__ == '__main__':
    ...
    #simgroup.add_argument('--id', metavar="id", type=int, help='Simulation number, useful for array jobs. Default is 0.', default=0)
    # parser.add_argument('--debug', action='store_true', help='print debug messages to stderr')
    # parser.add_argument('--verbose', help='Print simulation status to stdout.', action='store_true')
    #args = parser.parse_args()
    #
    # start(args.path, args.brownian_timestep, args.saving_timestep, args.swelling_rate, args.system_lifetime, args.packing_fraction, args.particle_number, args.aspect_ratio, args.radius, args.simulation_id,  args.verbose)

    start("./data", 0.01, 0.001, 0, 1000, 0.4, 32, 0, 1, 1, True)
