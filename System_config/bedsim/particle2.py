...
"""
try:
    from bedsim.cython2.coverlap import pyfAB, pyfdotAB, pyhAB, pyrC, pynC, pyvC, pydhAB, pyquat, tc_finder3, pytobody, pytospace #,  collision_time_c 
    USE_CYTHON = True
except ImportError:
...
#         print("PARTICLE UPDATE before", self._id,self.position,localtime)
        ...
        if deltaT != 0:
            ...
            self.position = self.cell.cellspace.system.boundary.unwrap(newpos) # consider boundary conditions # FIXME: this should be a cell crossing event!
...
#             print("newpos",self._id,newpos)
    ...
    def cell_cross(self, to_cell, crossing_point): # FIXME: add crossing point and update! should be integrated somehow in this manner due to collision time inaccuracies!
        """
...
#        if self.cell.particles: # This could be a problem if the cell is empty -A
    ...
    def __init__(self, **kwargs):
        ...
        self.pinned = (kwargs.pop('pinned', 0) == 1) # if 'pinned' is not set in HDF-file, set particle as not pinned by default
        if self.pinned:
            self.pin()
        ...
        self._id = int(kwargs.pop('id', Particle._particle_counter)) # if key 'id' is not present use internal counter # FIXME: ensure that this won't create duplicate ids
...
class Circle(Particle):
    ...
    def collision_time(self, with_circle):
        ...
        r = -self.cell.cellspace.system.boundary.delta_dir(r_term(self), r_term(with_circle)) #### FIXME: for 2x2 systems this may give the wrong distance vector against direction of movement
        ...
        pcp = lambda obj: obj.cell.cellspace.system.boundary.unwrap(r_term(obj) + v_term(obj)*tmin) # THIS IS NOT THE COLLISION POINT BUT THE POSITION OF PARTICLE 'obj'!!!!!
        delta = lambda obj1, obj2: self.cell.cellspace.system.boundary.delta_dir(pcp(obj1), pcp(obj2))
        cp = lambda obj1, obj2: obj1.cell.cellspace.system.boundary.unwrap(pcp(obj1) + obj1.radius * delta(obj1,obj2)/np.linalg.norm(delta(obj1,obj2)))
...
class Ellipse(Orientable):
    ...
    def mass(self):
        return np.float64(1) #np.pi * self.major * self.minor # FIXME: only valid for constant density # FIXME: proportionality constant...
    ...
    def collision(self, withEllipse, collisionPoint): # FIXME: NOTE withEllipse! Add support for other geometries!
        ...
        normal = collisionPoint[3:6] # gives the end point of the normal vector wrt to with the origin at rc, this has to be translated to the origin of the simbox
        lambda0 = collisionPoint[-1]
         
        
        normal_min = self.cell.cellspace.system.boundary.delta_dir(np.array([0.,0.,0.]),normal) # translated vector from rc to the simbox
...
#         rcx_body = lambda obj: pytobody(obj.angle,rcx(obj))
        ...
        e_parallel = lambda obj: self.cell.cellspace.system.boundary.delta_dir(np.array([0.,0.,0.]),np.cross(rcx(obj), e_perpendicular))
...
#         v_term = lambda obj: obj.velocity + np.cross(rcx(obj),obj.angvel) + np.matmul(Gamma(obj),-rcx(obj))
        ...
        Theta_term = lambda obj: obj.mass() *( obj.minor**2 + obj.minor**2 ) / 5. #lambda obj: 2.0 * obj.mass() * obj.major**2 / 5.0 #Ia,Ib
         
        vn = np.matmul(e_perpendicular.transpose(),v_term(withEllipse) - v_term(self)) #np.dot(e_perpendicular,v_term(withEllipse) - v_term(self))
        
        denom = (1./self.mass() + 1./withEllipse.mass() + (np.linalg.norm(e_parallel(self))**2)/Theta_term(self) + (np.linalg.norm(e_parallel(withEllipse))**2)/Theta_term(withEllipse))
...
#             withEllipse.position = r2shifted
            ...
            F0dot = self.overlap_potential(withEllipse,lambda1,1e-8) - self.overlap_potential(withEllipse,lambda0,0.)
...
#             print("delpab > 0")
    ...
    def collision_for_swelling(self, withEllipse, collisionPoint): # FIXME: NOTE withEllipse! Add support for other geometries!
            """
            Calculate and apply the results of a collision of the current 'Ellipse' with the 'withEllipse' at a certain 'collisionPoint'.
            @param withEllipse Reference to another Ellipse as collision target
            @param collisionPoint Coordinates of the collision point
    ...
    #         Q2 = 2.* (p2*p2.transpose() - s2*P2 + (s2*s2 - 0.5)*np.identity(3) )
            ...
            if swellrate is not None:
                ...
                P = lambda obj: np.array([[0,-p(obj)[2],p(obj)[1]],[p(obj)[2],0,-p(obj)[0]],[-p(obj)[1],p(obj)[0],0]])
                Q = lambda obj: 2.* (p(obj)*p(obj).transpose() - s(obj)*P(obj) + (s(obj)*s(obj) - 0.5)*np.identity(3) )
...
#         unit_vsum = vsum/np.linalg.norm(vsum)
        ...
        if not self.pinned and not withEllipse.pinned: # both not pinned
            m1 = self.mass()
            m2 = withEllipse.mass()
            self.velocity        = 1/(2 * m1) * ( np.dot(e1, (np.dot(m1 * v1 + m2 * v2, e1) + np.linalg.norm(m1 * v1 - m2 * v2))) + np.dot(e2, np.dot(m1 * v1 + m2 * v2, e2)) + np.dot(e3, np.dot(m1 * v1 + m2 * v2, e3)) )
            withEllipse.velocity = 1/(2 * m2) * ( np.dot(e1, (np.dot(m1 * v1 + m2 * v2, e1) - np.linalg.norm(m2 * v1 - m2 * v2))) + np.dot(e2, np.dot(m1 * v1 + m2 * v2, e2)) + np.dot(e3, np.dot(m1 * v1 + m2 * v2, e3)))
        elif self.pinned and not withEllipse.pinned: # only self pinned
            withEllipse.velocity = 0.5 * ( np.dot(e_parallel, (np.dot(v2, e_parallel) - np.linalg.norm(v2))) + np.dot(e_perpendicular, np.dot(v2, e_perpendicular)) )
        elif not self.pinned and withEllipse.pinned: # only withEllipse pinned
            self.velocity        = 0.5 * ( np.dot(e_parallel, (np.dot(v1, e_parallel) + np.linalg.norm(v1))) + np.dot(e_perpendicular, np.dot(v1, e_perpendicular)) )
        else: # both pinned
            pass
...
#                 print("angvel",self.angvel,withEllipse.angvel)
    ...
    """
    def overlap_potential(self, withEllipse, lambda0, time): # set f in constructor to cython version of python version if available
        raise NotImplementedError()
...
#                 print("F0",pyfAB(lambda0, self.major, withEllipse.major, self.minor, withEllipse.minor, self.minor2, withEllipse.minor2, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time))
            ...
            return pyfAB(lambda0, majorA, majorB, minorA, minorB, minorA, minorB, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time)
        else:
            return float('inf') # FIXME

    # derivative of overlap_potential by time
    def overlap_potential_dt(self, withEllipse, lambda0, time): # set h in constructor to cython version of python version if available
        raise NotImplementedError()
    def __fdot_cython(self, withEllipse, lambda0, time):
        ...
        return pyfdotAB(lambda0, majorA, majorB, minorA, minorB, minorA, minorB, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time, self.velocity, withEllipse.velocity, self.angvel, withEllipse.angvel)

    # derivative of overlap_potential by lambda0
    def overlap_potential_dlambda0(self, withEllipse, lambda0, time): # set h in constructor to cython version of python version if available
        raise NotImplementedError()
...
#         return pyhAB(lambda0, self.major, withEllipse.major, self.minor, withEllipse.minor, self.minor2, withEllipse.minor2, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time)
    ...
    def __fABdlambda_cython(self, withEllipse, lambda0, time):
        ...
        return pyhAB(lambda0, majorA, majorB, minorA, minorB, minorA, minorB, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time)

    def overlap_potential_d2lambda0(self, withEllipse, lambda0, time): # set h in constructor to cython version of python version if available
        raise NotImplementedError()
    ...
    def __fABd2lambda_cython(self, withEllipse, lambda0, time):
        ...
        return pydhAB(lambda0, majorA, majorB, minorA, minorB, minorA, minorB, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time)
    ...
    def __rC_cython(self, withEllipse, lambda0, time):
        ...
        if swellrate is not None:
            ...
            minorA = self.minor
            minorB = withEllipse.minor
        return pyrC(lambda0, majorA, majorB, minorA, minorB, minorA, minorB, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time)
    ...
    def __nC_cython(self, withEllipse, lambda0, time):
        ...
        return pynC(lambda0, majorA, majorB, minorA, minorB, minorA, minorB, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time)
    ...
    def __vC_cython(self, withEllipse, lambda0, time):
        ...
        return pyvC(lambda0, majorA, majorB, minorA, minorB, minorA, minorB, qnewA, qnewB, self.position + self.velocity * time, withEllipse.position + withEllipse.velocity * time)
...
#             print("calc 1,tstart = tend",self._id,withEllipse._id)
        ...
        if tstart < 1e-8 and F0 < 0 and self.overlap_potential(withEllipse, lambda0, tstart+1e-8) < 0: # in case lambda0 is for t=tstart
...
#             print("swell calc 1,tstart = tend",self._id,withEllipse._id)
        ...
        if tstart < 1e-8 and F0 < 0 and self.overlap_potential(withEllipse, lambda0, tstart+1e-8) < 0: # in case lambda0 is for t=tstart
            ...
            cp = np.append(cp3,lambda0)
            cpt = np.concatenate(([tstart],cp), axis=0)
            print("swell calc2, OVERLAP!",self._id,withEllipse._id,"major",self.major,self.minor,"tc",tstart,"F0",F0)
...
#     @property
    def volume(self):
        ...
        return 4.*np.pi*self.major*self.minor*self.minor2/3.
#  
#     @area.setter
#     def area(self, area):
#         """
